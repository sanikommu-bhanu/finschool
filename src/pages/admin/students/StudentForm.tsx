...
import { HiOutlineUser, HiOutlinePhone, HiOutlineMail, HiOutlineAcademicCap, HiOutlineHashtag, HiOutlineCurrencyRupee, HiOutlineChartPie } from 'react-icons/hi';
import {
  studentSchema,
...
export function StudentForm({ defaultValues, submitting, submitLabel, onSubmit }: StudentFormProps) {
  ...
  const {
    ...
    defaultValues: defaultValues ?? defaultStudentValues,
  });

  // Re-hydrate the form when switching between "add" and "edit this student" without unmounting the sheet.
  useEffect(() => {
    reset(defaultValues ?? defaultStudentValues);
  }, [defaultValues, reset]);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pt-2 pb-6">
      ...
      <div className="space-y-4">
        <h4 className="text-[10px] uppercase font-bold tracking-widest text-blush-700/60 dark:text-blush-200/50 ml-2">Student Information</h4>
        <FloatingInput 
          label="Full Name" 
          icon={HiOutlineUser} 
      ...
      <div className="space-y-4">
        <h4 className="text-[10px] uppercase font-bold tracking-widest text-blush-700/60 dark:text-blush-200/50 ml-2">Guardian Information</h4>
        <FloatingInput 
          label="Guardian Name" 
          icon={HiOutlineUser} 
          placeholder="Rohan Mehta"
          error={errors.guardian}
          value={watch('guardian')}
          {...register('guardian')} 
        />
      ...
      <div className="space-y-4">
        <h4 className="text-[10px] uppercase font-bold tracking-widest text-blush-700/60 dark:text-blush-200/50 ml-2">Financial & Academic</h4>
        ...
        <div className="grid grid-cols-2 gap-3">
          <FloatingInput 
            ...
            error={errors.feeDue}
            value={watch('feeDue')}
            {...register('feeDue', { valueAsNumber: true })} 
          />
